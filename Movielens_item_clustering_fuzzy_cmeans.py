...
def update_avg_rating(user_rating, labels):
    ...
    for i in range(0, n_users):
        ...
        for j in range(0, n_items):
            if user_rating[i][j] != 0:
                tmp[labels[j] - 1].append(user_rating[i][j])#find the cluster of each movie the user has rated and append the rating
...
def calculate_error(test, predicted_rating,cluster):
    ...
    for i in range(0, n_users):
        for j in range(0, n_items):
            if test[i][j] > 0:
                f.write("%d, %d, %.4f\n" % (i+1, j+1, predicted_rating[i][labels[j]-1]))
                y_true.append(test[i][j])
                y_pred.append(predicted_rating[i][labels[j]-1])
    f.close()
